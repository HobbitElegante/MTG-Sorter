...
- **Browse tab:** overview, card search (type to search — does not dump the full ~36k cache), availability (same columns as Inventory minus Colors), Scryfall bulk sync
...
2. Enter deck name and optional commander; use **+** for Partner, Companion, or Background if needed.
...
- Selected deck summary: dismantled shows free coverage as `{available}/{needed}` trackable cards (basics/tokens excluded from the denominator); armed shows “complete”; commander and secondary role are shown when set.
...
See [`handoff.md`](handoff.md) for full project state (**v0.3.17**, session closed 2026-07-24), product decisions, architecture notes, and next steps (v0.4+).
